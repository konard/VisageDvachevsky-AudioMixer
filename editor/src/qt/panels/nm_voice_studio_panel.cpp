...
void WaveformWidget::paintEvent(QPaintEvent *) {
  ...
  for (int x = 0; x < w; ++x) {
    ...
    if (endSample > static_cast<int64_t>(numSamples)) endSample = numSamples;
    ...
    for (int64_t i = startSample; i < endSample; ++i) {
      float sample = m_clip->samples[i];
      if (sample < minVal) minVal = sample;
      if (sample > maxVal) maxVal = sample;
    }

    // Map to screen coordinates
    int y1 = centerY - static_cast<int>(maxVal * (h / 2 - 4));
    int y2 = centerY - static_cast<int>(minVal * (h / 2 - 4));
...
void WaveformWidget::updatePeakCache() {
  ...
  int w = width();
  m_displayPeaks.resize(w * 2);  // min/max pairs
  ...
  for (int x = 0; x < w; ++x) {
    ...
    m_displayPeaks[x * 2] = minVal;
    m_displayPeaks[x * 2 + 1] = maxVal;
  }
}
...
void AudioProcessor::applyFades(std::vector<float> &samples,
  ...
  // Fade in
  if (fadeInMs > 0) {
    int fadeInSamples = static_cast<int>(fadeInMs * sampleRate / 1000.0f);
    ...
    for (int i = 0; i < fadeInSamples; ++i) {
      ...
      samples[i] *= std::sin(t * 3.14159f / 2.0f);
    }
  }

  // Fade out
  if (fadeOutMs > 0) {
    int fadeOutSamples = static_cast<int>(fadeOutMs * sampleRate / 1000.0f);
    ...
    for (int i = 0; i < fadeOutSamples; ++i) {
      ...
      samples[startIdx + i] *= std::cos(t * 3.14159f / 2.0f);
    }
  }
}
...
void AudioProcessor::applyNoiseGate(std::vector<float> &samples,
  ...
  int attackSamples = static_cast<int>(attackMs * sampleRate / 1000.0f);
  int releaseSamples = static_cast<int>(releaseMs * sampleRate / 1000.0f);
  ...
  for (size_t i = 0; i < samples.size(); ++i) {
    ...
    // Update envelope
    if (absVal > envelope) {
      envelope += (absVal - envelope) / std::max(1, attackSamples);
    } else {
      envelope -= (envelope - absVal) / std::max(1, releaseSamples);
    }
    ...
    if (envelope > threshold) {
      // Open gate
      if (gateGain < 1.0f) {
        gateGain += (1.0f - gateGain) / std::max(1, attackSamples);
      }
    } else {
      // Close gate
      if (gateGain > reduction) {
        gateGain -= (gateGain - reduction) / std::max(1, releaseSamples);
      }
    }
...
void NMVoiceStudioPanel::onTrimToSelection() {
  ...
  int sampleRate = m_clip->format.sampleRate;
  newEdit.trimStartSamples = static_cast<int64_t>(selStart * sampleRate);
  newEdit.trimEndSamples = static_cast<int64_t>(
      (m_clip->getDurationSeconds() - selEnd) * sampleRate);
...
void NMVoiceStudioPanel::onPresetSelected(int index) {
  ...
  applyPreset(m_presets[index].name);
}
      ...
      if (bitsPerSample == 16) {
        ...
        for (size_t i = 0; i < numSamples; ++i) {
          ...
          for (int ch = 0; ch < numChannels; ++ch) {
            sample += rawSamples[i * numChannels + ch] / 32768.0f;
          }
          m_clip->samples[i] = sample / numChannels;
        }
        ...
        for (size_t i = 0; i < numSamples; ++i) {
          float sample = 0.0f;
          for (int ch = 0; ch < numChannels; ++ch) {
            sample += rawSamples[i * numChannels + ch] / 2147483648.0f;
          }
          m_clip->samples[i] = sample / numChannels;
        }
      }

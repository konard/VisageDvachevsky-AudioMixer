...
void NMTutorialManager::displayCurrentStep() {
  ...
  m_overlay->showTutorialStep(
      step.id, step.title, step.content, step.anchorId, step.hintType,
      step.position, static_cast<int>(m_currentStepIndex),
      static_cast<int>(m_activeTutorial->steps.size()),
      step.showBackButton && m_currentStepIndex > 0, step.showSkipButton,
      step.showDontShowAgain);
